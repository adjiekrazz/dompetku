...
class Debt extends React.Component {
  ...
  getID() {
    ...
    if (this.props.debt.length === 0) {
      ...
      id = this.props.debt[this.props.debt.length - 1].id + 1;
    }
    return id;
  }

  handleSubmit(data) {
    this.props.addDebt({ id: this.getID(), ...data})
    ...
    return (
      <Row className="mt-4">
        <Col className="col-lg-12 mb-4">
          <Card>
            <Card.Body>
              <Card.Text>Total Hutang : {this.state.total}</Card.Text>
              <Table striped bordered hover size="sm" responsive>
                ...
                <tbody>
                  {this.props.debt.length > 0 ? this.props.debt.map((value, key) => {
                    return (
                      <tr key={key}>
  ...
  client() {
    return (
      <Row>
        <Col className="col-lg-12 mb-4">
          <h6>Total Hutang : {this.state.total}</h6>
          <Table striped bordered hover size="sm" responsive>
            <thead>
                <tr>
                  <th>Tanggal</th>
